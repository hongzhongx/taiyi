...
namespace taiyi { namespace utilities {
    ...
    {
        std::vector< std::string > default_appender( { "{\"appender\":\"stderr\",\"stream\":\"std_error\"}", "{\"appender\":\"p2p\",\"file\":\"logs/p2p/p2p.log\"}" } );
        std::string str_default_appender = boost::algorithm::join( default_appender, " " );
        
        std::vector< std::string > default_logger( { "{\"name\":\"default\",\"level\":\"info\",\"appender\":\"stderr\"}", "{\"name\":\"p2p\",\"level\":\"warn\",\"appender\":\"p2p\"}" } );
        std::string str_default_logger = boost::algorithm::join( default_logger, " " );
        
        options.add_options()
            ("log-appender", boost::program_options::value< std::vector< std::string > >()->composing()->default_value( default_appender, str_default_appender ), "Appender definition json: {\"appender\", \"stream\", \"file\"} Can only specify a file OR a stream" )
            ...
            ("log-logger", boost::program_options::value< std::vector< std::string > >()->composing()->default_value( default_logger, str_default_logger ), "Logger definition json: {\"name\", \"level\", \"appender\"}" )
        ;
    }
            ...
            for( string& s : all_appenders )
            {
                ...
                while ((pos = s.find("{", pos)) != std::string::npos)
                {
                    ...
                    if( appender.stream.length() )
                    {
                        ...
                        logging_config.appenders.push_back(fc::appender_config( appender.appender, "console", fc::variant( console_appender_config ) ) );
                    ...
                    else // validate ensures the is either a stream or file configured
                    {
                        ...
                        file_appender_config.rotation_interval = fc::minutes( 30 );
                        file_appender_config.rotation_limit = fc::days(3);
                        logging_config.appenders.push_back(fc::appender_config( appender.appender, "file", fc::variant( file_appender_config ) ) );
                        found_logging_config = true;
                    }
                }
            }
