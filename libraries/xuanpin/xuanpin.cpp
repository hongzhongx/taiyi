    ...
    namespace detail {
        ...
        public:
            ...
            variant info() const
            {
                ...
                result["proposal_adopted_votes_threshold"] = _remote_api->get_chain_properties().proposal_adopted_votes_threshold;
...
namespace taiyi { namespace xuanpin {
    ...
    baiyujing_api::legacy_signed_transaction xuanpin_api::release_contract(const account_name_type& owner, const string& name, bool broadcast)
    { try {
        FC_ASSERT( !is_locked() );

        release_contract_operation op;
        ...
        return my->sign_transaction( tx, broadcast );
    } FC_CAPTURE_AND_RETHROW( (owner)(name) ) }
