    ...
    namespace detail
    {
        ...
        public:
            ...
            typedef boost::multi_index_container<
                ...
                indexed_by<
                    ordered_non_unique<tag<last_seen_time_index>, member<potential_peer_record, fc::time_point_sec, &potential_peer_record::last_seen_time> >,
                    hashed_unique<tag<endpoint_index>, member<potential_peer_record, fc::ip::endpoint, &potential_peer_record::endpoint>, std::hash<fc::ip::endpoint> >
                >
            > potential_peer_set;
        ...
        void peer_database_impl::open(const fc::path& peer_database_filename)
        {
            ...
            if (fc::exists(_peer_database_filename))
            {
                try
                {
                    ...
                    if (_potential_peer_set.size() > TAIYI_NET_MAX_PEERDB_SIZE)
                    {
                        // prune database to a reasonable size
                        auto iter = _potential_peer_set.begin();
                        std::advance(iter, TAIYI_NET_MAX_PEERDB_SIZE);
