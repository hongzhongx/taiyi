...
namespace taiyi { namespace net
    ...
    void peer_connection::send_queueable_message(std::unique_ptr<queued_message>&& message_to_send)
    {
        ...
        if (_total_queued_messages_size > TAIYI_NET_MAXIMUM_QUEUED_MESSAGES_IN_BYTES)
        {
            elog("send queue exceeded maximum size of ${max} bytes (current size ${current} bytes)",
                 ("max", TAIYI_NET_MAXIMUM_QUEUED_MESSAGES_IN_BYTES)("current", _total_queued_messages_size));
            try
            {
                close_connection();
            }
    ...
    {
        VERIFY_CORRECT_THREAD();
        fc::time_point_sec oldest_inventory_to_keep(fc::time_point::now() - fc::minutes(TAIYI_NET_MAX_INVENTORY_SIZE_IN_MINUTES));
    ...
    {
        VERIFY_CORRECT_THREAD();
        return inventory_peer_advertised_to_us.size() > TAIYI_NET_MAX_INVENTORY_SIZE_IN_MINUTES * TAIYI_NET_MAX_TRX_PER_SECOND * 60;
    }
    ...
    bool peer_connection::is_inventory_advertised_to_us_list_full() const
    {
        ...
        // plus the maximum number of blocks that would be generated in TAIYI_NET_MAX_INVENTORY_SIZE_IN_MINUTES (plus one,
        // to give us some wiggle room)
        return inventory_peer_advertised_to_us.size() >
        TAIYI_NET_MAX_INVENTORY_SIZE_IN_MINUTES * TAIYI_NET_MAX_TRX_PER_SECOND * 60 +
        (TAIYI_NET_MAX_INVENTORY_SIZE_IN_MINUTES + 1) * 60 / TAIYI_BLOCK_INTERVAL;
    }
