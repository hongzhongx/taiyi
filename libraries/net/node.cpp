...
namespace taiyi { namespace net {
    ...
    namespace detail
    {
        ...
        {
        private:
            static const uint32_t cache_duration_in_blocks = TAIYI_NET_MESSAGE_CACHE_DURATION_IN_BLOCKS;
            ...
            struct message_info
            {
                ...
                message_info( const message_hash_type& message_hash, const message& message_body, uint32_t block_clock_when_received, const message_propagation_data& propagation_data, fc::uint160_t message_contents_hash ) :
            ...
            typedef boost::multi_index_container<
                message_info,
                bmi::indexed_by<
                    bmi::ordered_unique< bmi::tag<message_hash_index>, bmi::member<message_info, message_hash_type, &message_info::message_hash> >,
                    bmi::ordered_non_unique< bmi::tag<message_contents_hash_index>, bmi::member<message_info, fc::uint160_t, &message_info::message_contents_hash> >,
                    bmi::ordered_non_unique< bmi::tag<block_clock_index>, bmi::member<message_info, uint32_t, &message_info::block_clock_when_received> >
                >
        ...
        public:
            blockchain_tied_message_cache() : block_clock( 0 ) {}
            void block_accepted();
            void cache_message( const message& message_to_cache, const message_hash_type& hash_of_message_to_cache, const message_propagation_data& propagation_data, const fc::uint160_t& message_content_hash );
        ...
        void blockchain_tied_message_cache::block_accepted()
        {
            ...
            if( block_clock > cache_duration_in_blocks )
                _message_cache.get<block_clock_index>().erase(_message_cache.get<block_clock_index>().begin(), _message_cache.get<block_clock_index>().lower_bound(block_clock - cache_duration_in_blocks ) );
        }
        ...
        struct prioritized_item_id
        {
            ...
            {
                static_assert(taiyi::net::block_message_type > taiyi::net::trx_message_type, "block_message_type must be greater than trx_message_type for prioritized_item_ids to sort correctly");
...
#undef DECLARE_ACCUMULATOR
            ...
            public:
                ...
                call_statistics_collector(const char* method_name, call_stats_accumulator* execution_accumulator, call_stats_accumulator* delay_before_accumulator, call_stats_accumulator* delay_after_accumulator) :
        ...
        public:
            ...
            taiyi::protocol::chain_id_type get_chain_id() const override;
            bool has_item( const net::item_id& id ) override;
            void handle_message( const message& ) override;
            bool handle_block( const taiyi::net::block_message& block_message, bool sync_mode, std::vector<fc::uint160_t>& contained_transaction_message_ids ) override;
            void handle_transaction( const taiyi::net::trx_message& transaction_message ) override;
...
#define POTENTIAL_PEER_DATABASE_FILENAME "peers.json"
            ...
            typedef std::unordered_map<taiyi::net::block_id_type, fc::time_point> active_sync_requests_map;
            
            active_sync_requests_map              _active_sync_requests; /// list of sync blocks we've asked for from peers but have not yet received
            std::list<taiyi::net::block_message> _new_received_sync_items; /// list of sync blocks we've just received but haven't yet tried to process
            std::list<taiyi::net::block_message> _received_sync_items; /// list of sync blocks we've received, but can't yet process because we are still missing blocks that come earlier in the chain
            // @}
            ...
            void send_sync_block_to_node_delegate(const taiyi::net::block_message& block_message_to_send);
            void process_backlog_of_sync_blocks();
            void trigger_process_backlog_of_sync_blocks();
            void process_block_during_sync(peer_connection* originating_peer, const taiyi::net::block_message& block_message, const message_hash_type& message_hash);
            void process_block_during_normal_operation(peer_connection* originating_peer, const taiyi::net::block_message& block_message, const message_hash_type& message_hash);
            ...
            message_propagation_data   get_transaction_propagation_data( const taiyi::net::transaction_id_type& transaction_id );
            message_propagation_data   get_block_propagation_data( const taiyi::net::block_id_type& block_id );
...
# define VERIFY_CORRECT_THREAD() do {} while (0)
        ...
        node_impl::node_impl(const std::string& user_agent) :
            ...
            _recent_block_interval_in_seconds(TAIYI_BLOCK_INTERVAL),
            _user_agent_string(user_agent),
            _most_recent_blocks_accepted(TAIYI_NET_DEFAULT_MAX_CONNECTIONS),
        ...
        void node_impl::p2p_network_connect_loop()
        {
            while (!_p2p_network_connect_loop_done.canceled() && !_node_is_shutting_down)
            {
                try
                {
                    ...
                    try
                    {
                        _retrigger_connect_loop_promise = fc::promise<void>::ptr( new fc::promise<void>("taiyi::net::retrigger_connect_loop") );
                        if( is_wanting_new_connections() || !_add_once_node_list.empty() )
                        {
                            ...
                            _retrigger_connect_loop_promise->wait_until( fc::time_point::now() + fc::seconds(TAIYI_PEER_DATABASE_RETRY_DELAY ) );
                        }
                        else
                        {
        ...
        {
            VERIFY_CORRECT_THREAD();
            return std::find_if(_received_sync_items.begin(), _received_sync_items.end(), [&item_hash]( const taiyi::net::block_message& message ) { return message.block_id == item_hash; } ) != _received_sync_items.end() ||
            std::find_if(_new_received_sync_items.begin(), _new_received_sync_items.end(), [&item_hash]( const taiyi::net::block_message& message ) { return message.block_id == item_hash; } ) != _new_received_sync_items.end();
        }
        ...
        void node_impl::request_sync_item_from_peer( const peer_connection_ptr& peer, const item_hash_t& item_to_request )
        {
            ...
            item_id item_id_to_request( taiyi::net::block_message_type, item_to_request );
        ...
        void node_impl::request_sync_items_from_peer( const peer_connection_ptr& peer, const std::vector<item_hash_t>& items_to_request )
        {
            ...
            peer->send_message(fetch_items_message(taiyi::net::block_message_type, items_to_request));
        }
        ...
        void node_impl::fetch_sync_items_loop()
        {
            while( !_fetch_sync_items_loop_done.canceled() )
            {
                ...
                if( !_sync_items_to_fetch_updated )
                {
                    ...
                    _retrigger_fetch_sync_items_loop_promise = fc::promise<void>::ptr( new fc::promise<void>("taiyi::net::retrigger_fetch_sync_items_loop") );
        ...
        void node_impl::fetch_items_loop()
        {
            while (!_fetch_item_loop_done.canceled())
            {
                ...
                fc::time_point oldest_timestamp_to_fetch = fc::time_point::now() - fc::seconds(_recent_block_interval_in_seconds * TAIYI_NET_MESSAGE_CACHE_DURATION_IN_BLOCKS);
                ...
                typedef boost::multi_index_container<
                    ...
                    boost::multi_index::indexed_by<
                        boost::multi_index::ordered_unique<boost::multi_index::member<peer_and_items_to_fetch, peer_connection_ptr, &peer_and_items_to_fetch::peer> >,
                        boost::multi_index::ordered_non_unique<
                            boost::multi_index::tag<requested_item_count_index>,
                            boost::multi_index::const_mem_fun<peer_and_items_to_fetch, size_t, &peer_and_items_to_fetch::number_of_items>
                        >
                    >
                ...
                for (auto item_iter = _items_to_fetch.begin(); item_iter != _items_to_fetch.end();)
                {
                    ...
                    else
                    {
                        ...
                        for (auto peer_iter = items_by_peer.get<requested_item_count_index>().begin(); peer_iter != items_by_peer.get<requested_item_count_index>().end(); ++peer_iter)
                        {
                            ...
                            if (peer_iter->item_ids.size() < TAIYI_NET_MAX_ITEMS_PER_PEER_DURING_NORMAL_OPERATION &&
                                peer->inventory_peer_advertised_to_us.find(item_iter->item) != peer->inventory_peer_advertised_to_us.end())
                            {
                                if (item_iter->item.item_type == taiyi::net::trx_message_type && peer->is_transaction_fetching_inhibited())
                ...
                {
                    _retrigger_fetch_item_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("taiyi::net::retrigger_fetch_item_loop"));
        ...
        void node_impl::advertise_inventory_loop()
        {
            while (!_advertise_inventory_loop_done.canceled())
            {
                ...
                {
                    _retrigger_advertise_inventory_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("taiyi::net::retrigger_advertise_inventory_loop"));
        ...
        void node_impl::terminate_inactive_connections_loop()
        {
            ...
            // reconnect with the rest of the network, or it might just futher isolate us.
            {
                ...
                fc::time_point closing_disconnect_threshold = fc::time_point::now() - fc::seconds(TAIYI_NET_PEER_DISCONNECT_TIMEOUT);
                for( const peer_connection_ptr& closing_peer : _closing_connections )
                    ...
                    {
                        // we asked this peer to close their connectoin to us at least TAIYI_NET_PEER_DISCONNECT_TIMEOUT
                ...
                fc::time_point failed_terminate_threshold = fc::time_point::now() - fc::seconds(TAIYI_NET_FAILED_TERMINATE_TIMEOUT_SECONDS);
            ...
            if (!_node_is_shutting_down && !_terminate_inactive_connections_loop_done.canceled())
                _terminate_inactive_connections_loop_done = schedule_task(
                    [this](){ terminate_inactive_connections_loop(); },
        ...
        void node_impl::fetch_updated_peer_lists_loop()
        {
            ...
            fc::time_point_sec oldest_failed_ids_to_keep(fc::time_point::now() - fc::minutes(TAIYI_NET_PRUNE_FAILED_IDS_MINUTES));
            ...
            if (!_node_is_shutting_down && !_fetch_updated_peer_lists_loop_done.canceled() )
                _fetch_updated_peer_lists_loop_done = schedule_task(
                    [this](){ fetch_updated_peer_lists_loop(); },
                    fc::time_point::now() + fc::minutes(TAIYI_NET_FETCH_UPDATED_PEER_LISTS_INTERVAL_MINUTES),
        ...
        void node_impl::bandwidth_monitor_loop()
        {
            ...
            if (!_node_is_shutting_down && !_bandwidth_monitor_loop_done.canceled())
                _bandwidth_monitor_loop_done = schedule_task(
                    [=](){ bandwidth_monitor_loop(); },
                    fc::time_point::now() + fc::seconds(TAIYI_NET_BANDWIDTH_MONITOR_INTERVAL_SECONDS),
                    "bandwidth_monitor_loop"
                );
        }
        ...
        void node_impl::dump_node_status_task()
        {
            ...
            if (!_node_is_shutting_down && !_dump_node_status_task_done.canceled())
                _dump_node_status_task_done = schedule_task(
                    [=](){ dump_node_status_task(); },
                    fc::time_point::now() + fc::minutes(TAIYI_NET_DUMP_NODE_STATUS_INTERVAL_MINUTES),
                    "dump_node_status_task"
                );
        }
        ...
        bool node_impl::merge_address_info_with_potential_peer_database(const std::vector<address_info> addresses)
        {
            ...
            {
                if (address.firewalled == taiyi::net::firewalled_state::not_firewalled)
                {
        ...
        void node_impl::on_message( peer_connection* originating_peer, const message& received_message )
        {
            ...
            dlog("handling message ${type} ${hash} size ${size} from peer ${endpoint}",
                 ("type", taiyi::net::core_message_type_enum(received_message.msg_type))("hash", message_hash)
        ...
        void node_impl::parse_hello_user_data_for_peer(peer_connection* originating_peer, const fc::variant_object& user_data)
        {
            ...
            if (user_data.contains("taiyi_git_revision_sha"))
                originating_peer->taiyi_git_revision_sha = user_data["taiyi_git_revision_sha"].as_string();
            if (user_data.contains("taiyi_git_revision_unix_timestamp"))
                originating_peer->taiyi_git_revision_unix_timestamp = fc::time_point_sec(user_data["taiyi_git_revision_unix_timestamp"].as<uint32_t>());
            ...
            if (user_data.contains("chain_id"))
                originating_peer->chain_id = user_data["chain_id"].as<taiyi::protocol::chain_id_type>();
            else //add by xpeng, for conveting old version. TODO: remove this
            {
                originating_peer->chain_id = _delegate->get_chain_id();
            }
        }
        ...
        void node_impl::on_hello_message( peer_connection* originating_peer, const hello_message& hello_message_received )
        {
            ...
            if (originating_peer->last_known_fork_block_number == 0 &&
                originating_peer->taiyi_git_revision_unix_timestamp)
            {
                uint32_t unix_timestamp = originating_peer->taiyi_git_revision_unix_timestamp->sec_since_epoch();
            ...
            if (originating_peer->their_state == peer_connection::their_connection_state::just_connected)
            {
                if (hello_message_received.node_public_key != expected_node_public_key.serialize())
                {
                    ...
                    connection_rejected_message connection_rejected(_user_agent_string, core_protocol_version, originating_peer->get_socket().remote_endpoint(), rejection_reason_code::invalid_hello_message, rejection_message);
                ...
                if (originating_peer->last_known_fork_block_number != 0)
                {
                    ...
                    if (next_fork_block_number != 0)
                    {
                        ...
                        if (next_fork_block_number < head_block_num)
                        {
                            ...
                            ulog("Rejecting connection from peer because their version is too old.  Their version date: ${date}", ("date", originating_peer->taiyi_git_revision_unix_timestamp));
#endif
                            ...
                            connection_rejected_message connection_rejected(_user_agent_string, core_protocol_version, originating_peer->get_socket().remote_endpoint(), rejection_reason_code::unspecified, rejection_message.str() );
                ...
                {
                    wlog("Received hello message from peer running a node for different blockchain. my_chain_id=${my_chain_id}, their_chain_id=${their_chain_id}",
                    ...
                    connection_rejected_message connection_rejected(_user_agent_string, core_protocol_version, originating_peer->get_socket().remote_endpoint(), rejection_reason_code::different_chain, rejection_message.str() );
                ...
                if (already_connected_to_this_peer)
                {
                    ...
                    if (_node_id == originating_peer->node_id)
                        connection_rejected = connection_rejected_message(_user_agent_string, core_protocol_version, originating_peer->get_socket().remote_endpoint(), rejection_reason_code::connected_to_self, "I'm connecting to myself");
                    else
                        connection_rejected = connection_rejected_message(_user_agent_string, core_protocol_version, originating_peer->get_socket().remote_endpoint(), rejection_reason_code::already_connected, "I'm already connected to you");
                    originating_peer->their_state = peer_connection::their_connection_state::connection_rejected;
                    originating_peer->send_message(message(connection_rejected));
                ...
                {
                    connection_rejected_message connection_rejected(_user_agent_string, core_protocol_version, originating_peer->get_socket().remote_endpoint(), rejection_reason_code::blocked, "you are not in my allowed_peers list");
                    originating_peer->their_state = peer_connection::their_connection_state::connection_rejected;
                    originating_peer->send_message( message(connection_rejected ) );
                ...
                else
                {
                    ...
                    {
                        connection_rejected_message connection_rejected(_user_agent_string, core_protocol_version, originating_peer->get_socket().remote_endpoint(), rejection_reason_code::not_accepting_connections, "not accepting any more incoming connections");
                        originating_peer->their_state = peer_connection::their_connection_state::connection_rejected;
                        originating_peer->send_message(message(connection_rejected));
        ...
        void node_impl::on_connection_accepted_message(peer_connection* originating_peer, const connection_accepted_message& connection_accepted_message_received)
        {
            ...
            if (_is_firewalled == firewalled_state::unknown &&
                _last_firewall_check_message_sent < now - fc::minutes(TAIYI_NET_FIREWALL_CHECK_MESSAGE_INTERVAL_MINUTES) &&
        ...
        void node_impl::on_address_request_message(peer_connection* originating_peer, const address_request_message& address_request_message_received)
        {
            ...
            if (!_node_configuration.peer_advertising_disabled)
            {
                ...
                for (const peer_connection_ptr& active_peer : _active_connections)
                {
                    ...
                    reply.addresses.emplace_back(address_info(*active_peer->get_remote_endpoint(), fc::time_point::now(), active_peer->round_trip_delay, active_peer->node_id, active_peer->direction, active_peer->is_firewalled));
                }
            }
        ...
        void node_impl::on_address_message(peer_connection* originating_peer, const address_message& address_message_received)
        {
            ...
            std::vector<taiyi::net::address_info> updated_addresses = address_message_received.addresses;
        ...
        void node_impl::on_fetch_blockchain_item_ids_message(peer_connection* originating_peer, const fetch_blockchain_item_ids_message& fetch_blockchain_item_ids_message_received)
        {
            VERIFY_CORRECT_THREAD();
            ...
            if( reply_message.item_hashes_available.empty() )
                originating_peer->peer_needs_sync_items_from_us = false; /* I have no items in my blockchain */
                    ...
                    std::find(fetch_blockchain_item_ids_message_received.blockchain_synopsis.begin(), fetch_blockchain_item_ids_message_received.blockchain_synopsis.end(), reply_message.item_hashes_available.back() ) != fetch_blockchain_item_ids_message_received.blockchain_synopsis.end() )
            {
        ...
        void node_impl::on_blockchain_item_ids_inventory_message(peer_connection* originating_peer, const blockchain_item_ids_inventory_message& blockchain_item_ids_inventory_message_received )
        {
            VERIFY_CORRECT_THREAD();
            ...
            if( originating_peer->item_ids_requested_from_peer )
            {
                ...
                if (!blockchain_item_ids_inventory_message_received.item_hashes_available.empty())
                {
                    ...
                    if (synopsis_sent_in_request.empty())
                    {
                        ...
                        if (_delegate->get_block_number(first_item_hash) != 1)
                        {
                            ...
                            fc::exception error_for_peer(FC_LOG_MESSAGE(error, "You gave an invalid response for my request for sync blocks.  I asked for blocks starting from the beginning of the chain, but you returned a list of blocks starting with ${first_block}", ("first_block", first_item_hash)));
                    ...
                    else // synopsis was not empty, we expect a response building off one of the blocks we sent
                    {
                        if (boost::range::find(synopsis_sent_in_request, first_item_hash) == synopsis_sent_in_request.end())
                        {
                            wlog("Invalid response from peer ${peer_endpoint}.  We requested a list of sync blocks based on the synopsis ${synopsis}, but they "
                                 ...
                                 ("first_block", first_item_hash));
                            fc::exception error_for_peer(FC_LOG_MESSAGE(error, "You gave an invalid response for my request for sync blocks.  I asked for blocks following something in ${synopsis}, but you returned a list of blocks starting with ${first_block} which wasn't one of your choices",
                                ("synopsis", synopsis_sent_in_request)
                                ("first_block", first_item_hash)));
                            disconnect_from_peer(originating_peer, "You gave an invalid response to my request for sync blocks", true, error_for_peer);
                            return;
                ...
                try
                {
                    ...
                    originating_peer->number_of_unfetched_item_ids * TAIYI_BLOCK_INTERVAL;
                    if (minimum_time_of_last_offered_block > _delegate->get_blockchain_now() + TAIYI_NET_FUTURE_SYNC_BLOCKS_GRACE_PERIOD_SEC)
                    {
                    ...
                    if (blockchain_item_ids_inventory_message_received.total_remaining_item_count != 0)
                    {
                        ...
                        if (originating_peer->ids_of_items_to_get.size() > TAIYI_NET_MIN_BLOCK_IDS_TO_PREFETCH)
                        {
        ...
        void node_impl::on_fetch_items_message(peer_connection* originating_peer, const fetch_items_message& fetch_items_message_received)
        {
            ...
            {
                taiyi::net::block_message block = last_block_message_sent->as<taiyi::net::block_message>();
            ...
            for (const message& reply : reply_messages)
            {
                if (reply.msg_type == block_message_type)
                    originating_peer->send_item(item_id(block_message_type, reply.as<taiyi::net::block_message>().block_id));
                else
                    originating_peer->send_message(reply);
            }
        }
        ...
        void node_impl::on_item_not_available_message( peer_connection* originating_peer, const item_not_available_message& item_not_available_message_received )
        {
            ...
            if (sync_item_iter != originating_peer->sync_items_requested_from_peer.end())
            {
                ...
                if (originating_peer->peer_needs_sync_items_from_us)
                    ...
                    disconnect_from_peer(originating_peer, "You are missing a sync item you claim to have, your database is probably corrupted. Try --rebuild-index.",true, fc::exception(FC_LOG_MESSAGE(error,"You are missing a sync item you claim to have, your database is probably corrupted. Try --rebuild-index.", ("item_id", requested_item))));
        ...
        void node_impl::on_item_ids_inventory_message(peer_connection* originating_peer, const item_ids_inventory_message& item_ids_inventory_message_received)
        {
            ...
            for( const item_hash_t& item_hash : item_ids_inventory_message_received.item_hashes_available )
            {
                ...
                if (!we_advertised_this_item_to_a_peer)
                {
                    ...
                    if ((item_ids_inventory_message_received.item_type == taiyi::net::trx_message_type &&
                    ...
                    if (!we_requested_this_item_from_a_peer)
                    {
                        ...
                        {
                            dlog("not adding ${item_hash} to our list of items to fetch because we've recently fetched a copy and it failed to push", ("item_hash", item_hash));
                        }
                        else
                        {
                            ...
                            if (items_to_fetch_iter == _items_to_fetch.get<item_id_index>().end())
                            {
                                ...
                                dlog("adding item ${item_hash} from inventory message to our list of items to fetch", ("item_hash", item_hash));
                                trigger_fetch_items_loop();
                            }
                            else
                            {
        ...
        void node_impl::send_sync_block_to_node_delegate(const taiyi::net::block_message& block_message_to_send)
        {
            ...
            try
            {
                ...
                //if(bn % 1000 == 0)
                {
                    ilog("Successfully pushed sync block ${num} (id:${id}) by ${siming}",
                         ("num", bn)
                         ("id", block_message_to_send.block_id)
                         ("siming", block_message_to_send.block.siming));
                }
                    ...
                    if (is_fork_block)
                    {
                        ...
                        if (peer->last_known_fork_block_number != 0)
                        {
                                ...
                                next_fork_block_number <= block_message_to_send.block.block_num())
                            {
                                ...
                                peers_to_disconnect[peer] = std::make_pair(disconnect_reason_stream.str(), fc::oexception(fc::exception(FC_LOG_MESSAGE(error, "You need to upgrade your client due to hard fork at block ${block_number}", ("block_number", block_message_to_send.block.block_num())))));
#ifdef ENABLE_DEBUG_ULOGS
                                ulog("Disconnecting from peer during sync because their version is too old.  Their version date: ${date}", ("date", peer->taiyi_git_revision_unix_timestamp));
#endif
            ...
            do
            {
                     ...
                     ++received_block_iter)
                {
                    ...
                    if (potential_first_block)
                    {
                        ...
                        {
                            taiyi::net::block_message block_message_to_process = *received_block_iter;
                        ...
                        else
                        {
                            ...
                            for (const peer_connection_ptr& peer : _active_connections)
                            {
                                ...
                                {
                                    peer->ids_of_items_being_processed.erase(items_being_processed_iter);
                                    dlog("Removed item from ${endpoint}'s list of items being processed, still processing ${len} blocks", ("endpoint", peer->get_remote_endpoint())("len", peer->ids_of_items_being_processed.size()));
        ...
        void node_impl::process_block_during_sync( peer_connection* originating_peer, const taiyi::net::block_message& block_message_to_process, const message_hash_type& message_hash )
        {
        ...
        void node_impl::process_block_during_normal_operation( peer_connection* originating_peer, const taiyi::net::block_message& block_message_to_process, const message_hash_type& message_hash )
        {
            ...
            try
            {
                              ...
                              block_message_to_process.block_id) == _most_recent_blocks_accepted.end())
                {
                    ...
                    ilog("Successfully pushed block ${num} (id:${id}) by ${siming}",
                         ("num", block_message_to_process.block.block_num())
                         ("id", block_message_to_process.block_id)
                         ("siming", block_message_to_process.block.siming));
                ...
                if (is_hard_fork_block(block_number))
                {
...
#ifdef ENABLE_DEBUG_ULOGS
                                ulog("Disconnecting from peer because their version is too old.  Their version date: ${date}", ("date", peer->taiyi_git_revision_unix_timestamp));
#endif
                            }
                        }
                    }
                    if (!peers_to_disconnect.empty())
                    {
                        ...
                        disconnect_exception = fc::exception(FC_LOG_MESSAGE(error, "You need to upgrade your client due to hard fork at block ${block_number}", ("block_number", block_number)));
                    }
                }
            }
            catch (const fc::canceled_exception&)
        ...
        void node_impl::process_block_message(peer_connection* originating_peer, const message& message_to_process, const message_hash_type& message_hash)
        {
            ...
            taiyi::net::block_message block_message_to_process(message_to_process.as<taiyi::net::block_message>());
            auto item_iter = originating_peer->items_requested_from_peer.find(item_id(taiyi::net::block_message_type, message_hash));
                    ...
                    try
                    {
                        ...
                        if (originating_peer->idle())
                        {
                            ...
                            if (originating_peer->number_of_unfetched_item_ids > 0 &&
                                originating_peer->ids_of_items_to_get.size() < TAIYI_NET_MIN_BLOCK_IDS_TO_PREFETCH)
                                fetch_next_batch_of_item_ids_from_peer(originating_peer);
                            else
                                trigger_fetch_sync_items_loop();
                        }
            ...
            fc::exception detailed_error(FC_LOG_MESSAGE(error, "You sent me a block that I didn't ask for, block_id: ${block_id}",
                ("block_id", block_message_to_process.block_id)
                ("taiyi_git_revision_sha", originating_peer->taiyi_git_revision_sha)
                ("taiyi_git_revision_unix_timestamp", originating_peer->taiyi_git_revision_unix_timestamp)
        ...
        void node_impl::on_current_time_reply_message(peer_connection* originating_peer, const current_time_reply_message& current_time_reply_message_received)
        {
            ...
            originating_peer->clock_offset = fc::microseconds(((current_time_reply_message_received.request_received_time - current_time_reply_message_received.request_sent_time) + (current_time_reply_message_received.reply_transmitted_time - reply_received_time)).count() / 2);
            originating_peer->round_trip_delay = (reply_received_time - current_time_reply_message_received.request_sent_time) - (current_time_reply_message_received.reply_transmitted_time - current_time_reply_message_received.request_received_time);
        }
        ...
        void node_impl::on_get_current_connections_request_message(peer_connection* originating_peer, const get_current_connections_request_message& get_current_connections_request_message_received)
        {
            VERIFY_CORRECT_THREAD();
            ...
            for (const peer_connection_ptr& peer : _active_connections)
            {
                ...
                fc::mutable_variant_object user_data;
                if (peer->taiyi_git_revision_sha)
                    user_data["taiyi_git_revision_sha"] = *peer->taiyi_git_revision_sha;
                if (peer->taiyi_git_revision_unix_timestamp)
                    user_data["taiyi_git_revision_unix_timestamp"] = *peer->taiyi_git_revision_unix_timestamp;
        ...
        void node_impl::on_get_current_connections_reply_message(peer_connection* originating_peer, const get_current_connections_reply_message& get_current_connections_reply_message_received)
        {
            VERIFY_CORRECT_THREAD();
        }
        ...
        void node_impl::process_ordinary_message( peer_connection* originating_peer, const message& message_to_process, const message_hash_type& message_hash )
        {
            ...
            if( iter == originating_peer->items_requested_from_peer.end() )
            {
                wlog( "received a message I didn't ask for from peer ${endpoint}, disconnecting from peer",
                     ( "endpoint", originating_peer->get_remote_endpoint() ) );
                fc::exception detailed_error( FC_LOG_MESSAGE(error, "You sent me a message that I didn't ask for, message_hash: ${message_hash}", ( "message_hash", message_hash ) ) );
        ...
        void node_impl::load_configuration( const fc::path& configuration_directory )
        {
...
#ifdef TAIYI_TEST_NETWORK
                uint32_t port = TAIYI_NET_TEST_P2P_PORT + TAIYI_TEST_NETWORK_VERSION;
#else
                uint32_t port = TAIYI_NET_DEFAULT_P2P_PORT;
#endif
            ...
            try
            {
                ...
                for (peer_database::iterator itr = _potential_peer_db.begin(); itr != _potential_peer_db.end(); ++itr)
                {
                    ...
                    updated_peer_record.last_connection_attempt_time = std::min<fc::time_point_sec>(updated_peer_record.last_connection_attempt_time, fc::time_point::now() - fc::seconds(_node_configuration.peer_connection_retry_timeout));
                    _potential_peer_db.update_entry(updated_peer_record);
                }
                ...
                for( ;; )
                {
                    ...
                    if (listen_failed)
                    {
                        if( _node_configuration.wait_if_endpoint_is_busy )
                        {
                            ...
                            fc::usleep( fc::seconds(TAIYI_NET_PORT_WAIT_DELAY_SECONDS) );
                        }
        ...
        void node_impl::disconnect_from_peer( peer_connection* peer_to_disconnect, const std::string& reason_for_disconnect, bool caused_by_error /* = false */, const fc::oexception& error /* = fc::oexception() */ )
        {
            VERIFY_CORRECT_THREAD();
            ...
            if (caused_by_error)
            {
                ...
                error_message << "I am disconnecting peer " << fc::variant( peer_to_disconnect->get_remote_endpoint() ).as_string() << " for reason: " << reason_for_disconnect;
        ...
        void node_impl::broadcast( const message& item_to_broadcast, const message_propagation_data& propagation_data )
        {
            ...
            if( item_to_broadcast.msg_type == taiyi::net::block_message_type )
            {
                taiyi::net::block_message block_message_to_broadcast = item_to_broadcast.as<taiyi::net::block_message>();
            ...
            else if( item_to_broadcast.msg_type == taiyi::net::trx_message_type )
            {
                taiyi::net::trx_message transaction_message_to_broadcast = item_to_broadcast.as<taiyi::net::trx_message>();
        ...
        message_propagation_data node_impl::get_transaction_propagation_data( const taiyi::net::transaction_id_type& transaction_id )
        {
            VERIFY_CORRECT_THREAD();
            return _message_cache.get_message_propagation_data( transaction_id );
        }
        
        message_propagation_data node_impl::get_block_propagation_data( const taiyi::net::block_id_type& block_id )
        {
            VERIFY_CORRECT_THREAD();
...
return my->method_name(__VA_ARGS__)
    ...
    message_propagation_data node::get_transaction_propagation_data( const taiyi::net::transaction_id_type& transaction_id )
    {
        INVOKE_IN_IMPL(get_transaction_propagation_data, transaction_id);
    }
    
    message_propagation_data node::get_block_propagation_data( const taiyi::net::block_id_type& block_id )
    {
 ...
 */
        ...
        taiyi::protocol::chain_id_type statistics_gathering_node_delegate_wrapper::get_chain_id() const
        {
        ...
        bool statistics_gathering_node_delegate_wrapper::handle_block( const taiyi::net::block_message& block_message, bool sync_mode, std::vector<fc::uint160_t>& contained_transaction_message_ids)
        {
            INVOKE_AND_COLLECT_STATISTICS(handle_block, block_message, sync_mode, contained_transaction_message_ids);
        }
        
        void statistics_gathering_node_delegate_wrapper::handle_transaction( const taiyi::net::trx_message& transaction_message )
        {
            INVOKE_AND_COLLECT_STATISTICS(handle_transaction, transaction_message);
        }
        
        std::vector<item_hash_t> statistics_gathering_node_delegate_wrapper::get_block_ids(const std::vector<item_hash_t>& blockchain_synopsis, uint32_t& remaining_item_count, uint32_t limit /* = 2000 */)
        {
