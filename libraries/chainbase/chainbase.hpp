...
#define CHAINBASE_DEFAULT_CONSTRUCTOR( OBJECT_TYPE )    \
    ...
    public:
        ...
        generic_index( allocator<value_type> a, bfs::path p ) :_stack(a),_indices( a, p ),_size_of_value_type( sizeof(typename MultiIndexType::value_type) ),_size_of_this(sizeof(*this))
        {
            _revision = _indices.revision();
        }
        
        generic_index( allocator<value_type> a ) :_stack(a),_indices( a ),_size_of_value_type( sizeof(typename MultiIndexType::value_type) ),_size_of_this(sizeof(*this))
        {
            _revision = _indices.revision();
        }
    ...
    public:
        ...
        {
        public:
            session( session&& s ) : _index_sessions( std::move(s._index_sessions) ), _revision( s._revision ), _session_incrementer( s._session_incrementer )
            {}
            
            session( vector<std::unique_ptr<abstract_session>>&& s, int32_t& session_count ) : _index_sessions( std::move(s) ), _session_incrementer( session_count )
            {
        ...
        {
#if (!defined( __APPLE__ ) && !defined( WIN32 ))  // OS X does not support _SC_AVPHYS_PAGES
    ...
    private:
        ...
        void add_index_helper() {
            ...
            index_type* idx_ptr = new index_type( index_alloc() );
            idx_ptr->validate();
