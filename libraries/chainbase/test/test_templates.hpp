...
void basic_test( const std::vector< uint64_t >& v, std::function< void( Object& ) > call, chainbase::database& db )
{
...
template < typename IndexType, typename Object, typename Order >
void insert_remove_test( const std::vector< uint64_t >& v, std::function< void( Object& ) > call, chainbase::database& db )
{
    const auto& c = db.get_index< IndexType, Order >();
