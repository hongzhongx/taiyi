#pragma once
#include <protocol/sign_state.hpp>
...
namespace taiyi { namespace protocol {
    ...
    void verify_authority(const vector<AuthContainerType>& auth_containers, const flat_set<public_key_type>& sigs,
                          ...
                          uint32_t max_recursion_depth = TAIYI_MAX_SIG_CHECK_DEPTH,
    ...
    { try {
        ...
        if( required_posting.size() ) {
            ...
            TAIYI_ASSERT(!s.remove_unused_signatures(),
                         tx_irrelevant_sig,
        ...
        {
            TAIYI_ASSERT(s.check_authority(auth), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs));
        }
        ...
        for( const auto& id : required_active )
        {
            TAIYI_ASSERT(s.check_authority(id) ||
                         s.check_authority(get_owner(id)),
                         tx_missing_active_auth, "Missing Active Authority. id=${id}, auth=${auth}, sigs=${sigs}",
                         ("id",id)("auth",get_active(id))("owner",get_owner(id))("sigs", sigs));
        }
        
        for( const auto& id : required_owner )
        {
            TAIYI_ASSERT(owner_approvals.find(id) != owner_approvals.end() ||
                         s.check_authority(get_owner(id)),
