...
//
//     TAIYI_DEFINE_FIXED_STRING_IMPL( 32, fc::erpair< fc::uint128_t, fc::uint128_t > )
//
...
//
//     TAIYI_DEFINE_FIXED_STRING_IMPL( 32, BOOST_IDENTITY_TYPE((fc::erpair< fc::uint128_t, fc::uint128_t >)) )
//
#define TAIYI_DEFINE_FIXED_STRING_IMPL( SIZE, STORAGE_TYPE )       \
template<>                                                         \
struct fixed_string_impl_for_size< SIZE >                          \
{                                                                  \
    typedef taiyi::protocol::fixed_string_impl< STORAGE_TYPE > t;  \
};                                                                 \
                                                                   \
template<>                                                         \
...
{                                                                  \
    ...
    TAIYI_DEFINE_FIXED_STRING_IMPL( 16, BOOST_IDENTITY_TYPE((fc::uint128_t)) )
    TAIYI_DEFINE_FIXED_STRING_IMPL( 24, BOOST_IDENTITY_TYPE((fc::erpair< fc::uint128_t, uint64_t >)) )
    TAIYI_DEFINE_FIXED_STRING_IMPL( 32, BOOST_IDENTITY_TYPE((fc::erpair< fc::uint128_t, fc::uint128_t >)) )
...
namespace fc {
    ...
    namespace raw {
        ...
        inline void pack( Stream& s, const taiyi::protocol::fixed_string_impl< Storage >& u )
        {
            pack( s, std::string( u ) );
        }
        
        template< typename Stream, typename Storage >
        inline void unpack( Stream& s, taiyi::protocol::fixed_string_impl< Storage >& u, uint32_t depth )
        {
            depth++;
            std::string str;
            unpack( s, str, depth );
    ...
    void to_variant( const taiyi::protocol::fixed_string_impl< Storage >& s, variant& v )
    {
        v = std::string( s );
    }
    
    template< typename Storage >
    void from_variant( const variant& v, taiyi::protocol::fixed_string_impl< Storage >& s )
    {
        s = v.as_string();
    }

    template< typename Storage >
    struct get_typename< taiyi::protocol::fixed_string_impl< Storage > >
    {
        static const char* name()
        {
            static const std::string n =
            std::string("taiyi::protocol::fixed_string<") +
            std::to_string( taiyi::protocol::fixed_string_size_for_impl<Storage>::size ) +
            std::string(">");
            return n.c_str();
        }
    };
