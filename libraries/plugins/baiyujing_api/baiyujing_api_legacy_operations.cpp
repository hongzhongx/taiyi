...
namespace fc {
    ...
    void to_variant( const taiyi::plugins::baiyujing_api::legacy_operation& var,  fc::variant& vo )
    {
        var.visit( from_operation( vo ) );
    }
    
    void from_variant( const fc::variant& var, taiyi::plugins::baiyujing_api::legacy_operation& vo )
    {
        static std::map<string,int64_t> to_tag = []()
        {
            std::map<string,int64_t> name_map;
            for( int i = 0; i < taiyi::plugins::baiyujing_api::legacy_operation::count(); ++i )
            {
                taiyi::plugins::baiyujing_api::legacy_operation tmp;
                tmp.set_which(i);
                string n;
