...
namespace taiyi { namespace plugins { namespace debug_node {
    ...
    namespace detail
    {
        ...
        debug_node_plugin_impl::debug_node_plugin_impl() : _db( appbase::app().get_plugin< chain::chain_plugin >().db() ) {}
    ...
    void debug_node_plugin::set_program_options( options_description& cli, options_description& cfg )
    {
        cfg.add_options()
    ...
    void debug_node_plugin::plugin_initialize( const variables_map& options )
    {
        ...
        my->_post_apply_block_conn = my->_db.add_post_apply_block_handler( [this](const chain::block_notification& note){ on_post_apply_block(note); }, *this, 0 );
    }
    ...
    void debug_apply_update( chain::database& db, const fc::variant_object& vo, bool logging )
    {
       ...
       switch( action )
       {
          ...
          case db_action_write:
             db.modify( db.get_object( oid ), [&]( taiyi::db::object& obj )
             {
    ...
    */
    ...
    void debug_node_plugin::debug_generate_blocks( debug_generate_blocks_return& ret, const debug_generate_blocks_args& args )
    {
        ...
        if( args.debug_key != "" )
        {
            debug_private_key = taiyi::utilities::wif_to_key( args.debug_key );
        ...
        while( produced < args.count )
        {
            ...
            std::string scheduled_siming_name = db.get_scheduled_siming( slot );
            fc::time_point_sec scheduled_time = db.get_slot_time( slot );
            const chain::siming_object& scheduled_siming = db.get_siming( scheduled_siming_name );
            chain::public_key_type scheduled_key = scheduled_siming.signing_key;
            if( logging )
            {
            ...
            if( scheduled_key != debug_public_key )
            {
                ...
                {
                    if( logging ) wlog( "Modified key for siming ${w}", ("w", scheduled_siming_name) );
                    debug_update( [=]( chain::database& db )
                                 {
                        db.modify( db.get_siming( scheduled_siming_name ), [&]( chain::siming_object& w )
                                  {
            ...
            bp.generate_block( scheduled_time, scheduled_siming_name, *debug_private_key, args.skip );
            ++produced;
            slot = new_slot;
        }
