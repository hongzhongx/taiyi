...
#define TAIYI_DEBUG_NODE_PLUGIN_NAME "debug_node"
...
namespace taiyi { namespace plugins { namespace debug_node {
    ...
    struct debug_generate_blocks_args
    {
        ...
        uint32_t                                  count = 0;
        uint32_t                                  skip = taiyi::chain::database::skip_nothing;
    ...
    public:
        ...
        static const std::string& name() { static std::string name = TAIYI_DEBUG_NODE_PLUGIN_NAME; return name; }
        ...
        void debug_update( Lambda&& callback, uint32_t skip = taiyi::chain::database::skip_nothing )
        {
            // this was a method on database in Taiyi
        ...
        void debug_generate_blocks( debug_generate_blocks_return& ret, const debug_generate_blocks_args& args );
        uint32_t debug_generate_blocks( const std::string& debug_key, uint32_t count, uint32_t skip = taiyi::chain::database::skip_nothing, uint32_t miss_blocks = 0 );
        uint32_t debug_generate_blocks_until( const std::string& debug_key, const fc::time_point_sec& head_block_time, bool generate_sparsely, uint32_t skip = taiyi::chain::database::skip_nothing );
    ...
    private:
        void on_post_apply_block( const taiyi::chain::block_notification& note );
...
FC_REFLECT( taiyi::plugins::debug_node::debug_generate_blocks_args, (debug_key)(count)(skip)(miss_blocks)(edit_if_needed) )
FC_REFLECT( taiyi::plugins::debug_node::debug_generate_blocks_return, (blocks) )
