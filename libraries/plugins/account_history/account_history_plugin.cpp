...
#define TAIYI_NAMESPACE_PREFIX "taiyi::protocol::"
...
namespace taiyi { namespace plugins { namespace account_history {
    ...
    using taiyi::protocol::signed_block_header;
    using taiyi::protocol::signed_transaction;
    
    using taiyi::chain::operation_notification;
    using taiyi::chain::transaction_id_type;
    
    using taiyi::utilities::benchmark_dumper;
...
#define checkStatus(s) FC_ASSERT((s).ok(), "Data access failed: ${m}", ("m", (s).ToString()))
    ...
    class account_history_plugin::impl final
    {
    public:
        impl( account_history_plugin& self, const bpo::variables_map& options, const bfs::path& storagePath) : _self(self), _mainDb(appbase::app().get_plugin<taiyi::plugins::chain::chain_plugin>().db()), _storagePath(storagePath), _writeBuffer(_storage, _columnHandles)
        {
            collectOptions(options);
            
            _mainDb.add_pre_reindex_handler([&]( const taiyi::chain::reindex_notification& note ) -> void {
                on_pre_reindex( note );
            }, _self, 0);
            
            _mainDb.add_post_reindex_handler([&]( const taiyi::chain::reindex_notification& note ) -> void {
                on_post_reindex( note );
            }, _self, 0);
            
            TAIYI_ADD_PLUGIN_INDEX(_mainDb, volatile_operation_index);
        }
        ...
        void openDb()
        {
            ...
            if(status.ok())
            {
                ...
                const auto& rocksdb_plugin = appbase::app().get_plugin< account_history_plugin >();
                ...
                _on_post_apply_operation_con = _mainDb.add_post_apply_operation_handler([&]( const operation_notification& note ) {
            ...
            {
                elog("RocksDB cannot open database at location: `${p}'.\nReturned error: ${e}", ("p", strPath)("e", status.ToString()));
            }
        }

        void printReport(uint32_t blockNo, const char* detailText) const;
        void on_pre_reindex( const taiyi::chain::reindex_notification& note );
        void on_post_reindex( const taiyi::chain::reindex_notification& note );
        ...
        uint32_t enumVirtualOperationsFromBlockRange(uint32_t blockRangeBegin, uint32_t blockRangeEnd, std::function<void(const rocksdb_operation_object&)> processor) const;
    ...
    private:
        ...
        void prunePotentiallyTooOldItems(account_history_info* ahInfo, const account_name_type& name, const fc::time_point_sec& now);
        ...
        void loadSeqIdentifiers(DB* storageDb)
        {
            ...
            ilog("Loaded OperationObject seqId: ${o}, AccountHistoryObject seqId: ${ah}.", ("o", _operationSeqId)("ah", _accountHistorySeqId));
        }
        ...
        void storeOpFilteringParameters(const std::vector<std::string>& opList, flat_set<std::string>* storage) const;
    ...
    void account_history_plugin::impl::collectOptions(const boost::program_options::variables_map& options)
    {
        fc::mutable_variant_object state_opts;
        
        typedef std::pair< account_name_type, account_name_type > pairstring;
        TAIYI_LOAD_VALUE_SET(options, "account-history-track-account-range", _tracked_accounts, pairstring);
        
        state_opts[ "account-history-track-account-range" ] = _tracked_accounts;
        ...
        if(options.count("account-history-whitelist-ops"))
        {
            ...
            if( _op_list.size() )
            {
                state_opts["account-history-whitelist-ops"] = _op_list;
            }
        }
        ...
        if(options.count("account-history-blacklist-ops"))
        {
            ...
            {
                state_opts["account-history-blacklist-ops"] = _blacklisted_op_list;
            }
        }
    ...
    inline bool account_history_plugin::impl::isTrackedAccount(const account_name_type& name) const
    {
         ...
         */
        if(itr != _tracked_accounts.begin() && ((itr != _tracked_accounts.end() && itr->first != name ) || itr == _tracked_accounts.end()))
        {
            --itr;
        }
    ...
    std::vector<account_name_type> account_history_plugin::impl::getImpactedAccounts(const operation& op) const
    {
        flat_set<account_name_type> impacted;
        taiyi::chain::operation_get_impacted_accounts(op, impacted);
    ...
    inline bool account_history_plugin::impl::isTrackedOperation(const operation& op) const
    {
    ...
    void account_history_plugin::impl::storeOpFilteringParameters(const std::vector<std::string>& opList, flat_set<std::string>* storage) const
    {
        for(const auto& arg : opList)
        {
            ...
            {
                if( op.empty() == false )
                    storage->insert( TAIYI_NAMESPACE_PREFIX + op );
            }
        }
    }
    ...
    bool account_history_plugin::impl::find_operation_object(size_t opId, rocksdb_operation_object* op) const
    {
        std::string data;
    ...
    void account_history_plugin::impl::find_operations_by_block(size_t blockNum, std::function<void(const rocksdb_operation_object&)> processor) const
    {
    ...
    uint32_t account_history_plugin::impl::enumVirtualOperationsFromBlockRange(uint32_t blockRangeBegin, uint32_t blockRangeEnd, std::function<void(const rocksdb_operation_object&)> processor) const
    {
    ...
    bool account_history_plugin::impl::find_transaction_info(const protocol::transaction_id_type& trxId, uint32_t* blockNo, uint32_t* txInBlock) const
    {
        ReadOptions rOptions;
    ...
    uint32_t account_history_plugin::impl::get_lib()
    {
        std::string data;
    ...
    void account_history_plugin::impl::update_lib( uint32_t lib )
    {
        auto s = _writeBuffer.Put( _columnHandles[ CURRENT_LIB ], LIB_ID, lib_slice_t( lib ) );
        checkStatus( s );
    }

    account_history_plugin::impl::ColumnDefinitions account_history_plugin::impl::prepareColumnDefinitions(bool addDefaultColumn)
    {
    ...
    bool account_history_plugin::impl::createDbSchema(const bfs::path& path)
    {
        DB* db = nullptr;
    ...
    void account_history_plugin::impl::buildAccountHistoryRecord( const account_name_type& name, const rocksdb_operation_object& obj )
    {
        ...
        if(found)
        {
            ...
            if(_prune && count > ACCOUNT_HISTORY_LENGTH_LIMIT && ((obj.timestamp - ahInfo.oldestEntryTimestamp) > fc::days(ACCOUNT_HISTORY_TIME_LIMIT)))
            {
    ...
    void account_history_plugin::impl::storeTransactionInfo(const chain::transaction_id_type& trx_id, uint32_t blockNo, uint32_t trx_in_block)
    {
    ...
    void account_history_plugin::impl::prunePotentiallyTooOldItems(account_history_info* ahInfo, const account_name_type& name, const fc::time_point_sec& now)
    {
        std::string strName = name;
        
        auto ageLimit =  fc::days(ACCOUNT_HISTORY_TIME_LIMIT);
        
        ah_op_by_id_slice_t oldestEntrySlice(std::make_pair(ahInfo->id, ahInfo->oldestEntryId));
        auto lookupUpperBound = std::make_pair(ahInfo->id + 1, ahInfo->newestEntryId - ACCOUNT_HISTORY_LENGTH_LIMIT + 1);
        ...
        for(; dataItr->Valid(); dataItr->Next())
        {
            ...
            if(age > ageLimit)
            {
                ...
                ah_op_by_id_slice_t rightBoundarySlice(std::make_pair(ahInfo->id, rightBoundary));
                s = _writeBuffer.SingleDelete(_columnHandles[AH_INFO_BY_NAME], rightBoundarySlice);
                checkStatus(s);
            }
            else
            {
    ...
    void account_history_plugin::impl::on_pre_reindex(const taiyi::chain::reindex_notification& note)
    {
    ...
    void account_history_plugin::impl::on_post_reindex(const taiyi::chain::reindex_notification& note)
    {
        ilog("Reindex completed up to block: ${b}. Setting back write limit to non-massive level.", ("b", note.last_block_number));
    ...
    void account_history_plugin::impl::printReport(uint32_t blockNo, const char* detailText) const
    {
    ...
    void account_history_plugin::impl::importData(unsigned int blockLimit)
    {
        if(_storage == nullptr)
        {
        ...
        _mainDb.foreach_operation([blockLimit, &blockNo, &lastBlock, this](const signed_block_header& prevBlockHeader, const signed_block& block, const signed_transaction& tx, uint32_t txInBlock, const operation& op, uint16_t opInTx) -> bool
                                  {
    ...
    void account_history_plugin::impl::on_irreversible_block( uint32_t block_num )
    {
    ...
    void account_history_plugin::set_program_options(boost::program_options::options_description &command_line_options, boost::program_options::options_description &cfg)
    {
        cfg.add_options()
        ("account-history-path", bpo::value<bfs::path>()->default_value("blockchain/account-history-storage"),
    ...
    {
        if(options.count("account-history-stop-import-at-block"))
        ...
        if(options.count("account-history-path"))
            dbPath = options.at("account-history-path").as<bfs::path>();
    ...
    {
        ilog("Starting up account_history_plugin...");
    ...
    {
        ilog("Shutting down account_history_plugin...");
        _my->shutdownDb();
    }
    ...
    bool account_history_plugin::find_operation_object(size_t opId, rocksdb_operation_object* op) const
    {
        return _my->find_operation_object(opId, op);
    }
    
    void account_history_plugin::find_operations_by_block(size_t blockNum, std::function<void(const rocksdb_operation_object&)> processor) const
    {
        _my->find_operations_by_block(blockNum, processor);
    }

    uint32_t account_history_plugin::enum_operations_from_block_range(uint32_t blockRangeBegin, uint32_t blockRangeEnd, std::function<void(const rocksdb_operation_object&)> processor) const
    {
        return _my->enumVirtualOperationsFromBlockRange(blockRangeBegin, blockRangeEnd, processor);
    }

    bool account_history_plugin::find_transaction_info(const protocol::transaction_id_type& trxId, uint32_t* blockNo, uint32_t* txInBlock) const
    {
...
FC_REFLECT( taiyi::plugins::account_history::account_history_info, (id)(oldestEntryId)(newestEntryId)(oldestEntryTimestamp) )
