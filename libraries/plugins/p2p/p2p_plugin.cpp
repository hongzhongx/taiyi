...
#include <plugins/p2p/p2p_default_seeds.hpp>
...
namespace taiyi { namespace plugins { namespace p2p {
    ...
    using taiyi::net::item_hash_t;
    using taiyi::net::item_id;
    using taiyi::net::message;
    using taiyi::net::block_message;
    using taiyi::net::trx_message;
    
    using taiyi::protocol::block_header;
    ...
    namespace detail {
        ...
        class p2p_plugin_impl : public taiyi::net::node_delegate
        {
        public:
            
            p2p_plugin_impl( plugins::chain::chain_plugin& c ) : running(true), activeHandleBlock(false), activeHandleTx(false), chain( c )
            {
            ...
            virtual taiyi::protocol::chain_id_type get_chain_id() const override;
            
            // node_delegate interface
            virtual bool has_item( const taiyi::net::item_id& ) override;
            virtual bool handle_block( const taiyi::net::block_message&, bool, std::vector<fc::uint160_t>& ) override;
            virtual void handle_transaction( const taiyi::net::trx_message& ) override;
            virtual void handle_message( const taiyi::net::message& ) override;
            virtual std::vector< taiyi::net::item_hash_t > get_block_ids( const std::vector< taiyi::net::item_hash_t >&, uint32_t&, uint32_t ) override;
            virtual taiyi::net::message get_item( const taiyi::net::item_id& ) override;
            virtual std::vector< taiyi::net::item_hash_t > get_blockchain_synopsis( const taiyi::net::item_hash_t&, uint32_t ) override;
            virtual void sync_status( uint32_t, uint32_t ) override;
            virtual void connection_count_changed( uint32_t ) override;
            virtual uint32_t get_block_number( const taiyi::net::item_hash_t& ) override;
            virtual fc::time_point_sec get_block_time(const taiyi::net::item_hash_t& ) override;
            virtual fc::time_point_sec get_blockchain_now() override;
            virtual taiyi::net::item_hash_t get_head_block_id() const override;
            ...
            std::unique_ptr<taiyi::net::node> node;
        ...
        private:
            ...
            {
            public:
                shutdown_helper(p2p_plugin_impl& impl, std::atomic_bool& activityFlag, handler_state& barrier) : _impl(impl), _barrier(barrier), _activityFlag(activityFlag)
                {
        ...
        bool p2p_plugin_impl::has_item( const taiyi::net::item_id& id )
        {
            return chain.db().with_read_lock( [&]()
                                             {
                try
                {
                    if( id.item_type == taiyi::net::block_message_type )
        ...
        bool p2p_plugin_impl::handle_block( const taiyi::net::block_message& blk_msg, bool sync_mode, std::vector<fc::uint160_t>& )
        { try {
            ...
            if( running.load() )
            {
                ...
                try {
                    ...
                    // translate to a taiyi::net exception
                    fc_elog(fc::logger::get("sync"),
                            "Error when pushing block, current head block is ${head}:\n${e}",
                            ("e", e.to_detail_string())
                            ("head", head_block_num));
                    elog("Error when pushing block:\n${e}", ("e", e.to_detail_string()));
                    FC_THROW_EXCEPTION(taiyi::net::unlinkable_block_exception, "Error when pushing block:\n${e}", ("e", e.to_detail_string()));
                }
                    ...
                    elog("Error when pushing block:\n${e}", ("e", e.to_detail_string()));
                    if (e.code() == 4080000) {
                        elog("Rethrowing as taiyi::net exception");
                        FC_THROW_EXCEPTION(taiyi::net::unlinkable_block_exception, "Error when pushing block:\n${e}", ("e", e.to_detail_string()));
                    }
                    else {
                        throw;
        ...
        void p2p_plugin_impl::handle_transaction( const taiyi::net::trx_message& trx_msg )
        {
            if(running.load())
            {
                try
        ...
        void p2p_plugin_impl::handle_message( const taiyi::net::message& message_to_process )
        {
            // not a transaction, not a block
            FC_THROW( "Invalid Message Type" );
        }

        std::vector< taiyi::net::item_hash_t > p2p_plugin_impl::get_block_ids( const std::vector< taiyi::net::item_hash_t >& blockchain_synopsis, uint32_t& remaining_item_count, uint32_t limit )
        { try {
            return chain.db().with_read_lock( [&]()
                                             {
                ...
                else
                {
                    ...
                    if (!found_a_block_in_synopsis)
                        FC_THROW_EXCEPTION(taiyi::net::peer_is_on_an_unreachable_fork, "Unable to provide a list of blocks starting at any of the blocks in peer's synopsis");
                }
        ...
        taiyi::net::message p2p_plugin_impl::get_item( const taiyi::net::item_id& id )
        { try {
            if( id.item_type == taiyi::net::block_message_type )
            {
                return chain.db().with_read_lock( [&]()
                    ...
                    if( !opt_block )
                        elog("Couldn't find block ${id} -- corresponding ID in our chain is ${id2}", ("id", id.item_hash)("id2", chain.db().get_block_id_for_num(block_header::num_from_id(id.item_hash))));
        ...
        taiyi::protocol::chain_id_type p2p_plugin_impl::get_chain_id() const
        {
            return chain.db().get_chain_id();
        }
        
        std::vector< taiyi::net::item_hash_t > p2p_plugin_impl::get_blockchain_synopsis( const taiyi::net::item_hash_t& reference_point, uint32_t number_of_blocks_after_reference_point )
        { try {
                ...
                if (reference_point != item_hash_t())
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if (last_non_fork_block == block_id_type()) // if the fork goes all the way back to genesis (does taiyi's fork db allow this?)
                        ...
                        if (non_fork_high_block_num < low_block_num)
                        {
                            ...
                            FC_THROW_EXCEPTION(taiyi::net::block_older_than_undo_history, "Peer is are on a fork I'm unable to switch to");
                        }
                    }
                }
                else
        ...
        uint32_t p2p_plugin_impl::get_block_number( const taiyi::net::item_hash_t& block_id )
        { try {
            return block_header::num_from_id(block_id);
        } FC_CAPTURE_AND_RETHROW( (block_id) ) }

        fc::time_point_sec p2p_plugin_impl::get_block_time( const taiyi::net::item_hash_t& block_id )
        { try {
            return chain.db().with_read_lock( [&]() {
        ...
        taiyi::net::item_hash_t p2p_plugin_impl::get_head_block_id() const
        { try {
            return chain.db().with_read_lock( [&]() {
        ...
        {
            return 0; // there are no forks in taiyi
        }
    ...
    void p2p_plugin::set_program_options( bpo::options_description& cli, bpo::options_description& cfg)
    {
        ...
        cfg.add_options()
            ...
            ("p2p-parameters", bpo::value<string>(), ("P2P network parameters. (Default: " + fc::json::to_string(taiyi::net::node_configuration()) + " )").c_str() )
        ;
        cli.add_options()
    ...
    void p2p_plugin::plugin_initialize(const boost::program_options::variables_map& options)
    {
        ...
        my->user_agent = "Taiyi Reference Implementation";
    ...
    void p2p_plugin::plugin_startup()
    {
        my->p2p_thread.async( [this] {
            my->node.reset(new taiyi::net::node(my->user_agent));
            ...
            for( const auto& seed : my->seeds )
            {
                ...
                catch( taiyi::net::already_connected_to_requested_peer& )
                {
            ...
            my->node->sync_from(taiyi::net::item_id(taiyi::net::block_message_type, block_id), std::vector<uint32_t>());
    ...
    void p2p_plugin::broadcast_block( const taiyi::protocol::signed_block& block )
    {
        ulog("Broadcasting block #${n}", ("n", block.block_num()));
        my->node->broadcast( taiyi::net::block_message( block ) );
    }
    
    void p2p_plugin::broadcast_transaction( const taiyi::protocol::signed_transaction& tx )
    {
        ulog("Broadcasting tx #${n}", ("id", tx.id()));
        my->node->broadcast( taiyi::net::trx_message( tx ) );
    }
