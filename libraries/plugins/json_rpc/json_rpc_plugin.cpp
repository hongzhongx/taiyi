...
namespace taiyi { namespace plugins { namespace json_rpc {
    ...
    namespace detail
    {
        struct json_rpc_error
        {
            ...
            json_rpc_error( int32_t c, std::string m, fc::optional< fc::variant > d = fc::optional< fc::variant >() ) : code( c ), message( m ), data( d ) {}
    ...
    string json_rpc_plugin::call( const string& message )
    {
        ...
        {
            json_rpc_response response;
            response.error = json_rpc_error( JSON_RPC_SERVER_ERROR, "Unknown exception", fc::variant(fc::unhandled_exception( FC_LOG_MESSAGE( warn, "Unknown Exception" ), std::current_exception() ).to_detail_string() ) );
            return fc::json::to_string( response );
        }
    }
...
FC_REFLECT( taiyi::plugins::json_rpc::detail::json_rpc_error, (code)(message)(data) )
FC_REFLECT( taiyi::plugins::json_rpc::detail::json_rpc_response, (jsonrpc)(result)(error)(id) )

FC_REFLECT( taiyi::plugins::json_rpc::detail::get_signature_args, (method) )
