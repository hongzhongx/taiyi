...
#define TAIYI_JSON_RPC_PLUGIN_NAME "json_rpc"

#define JSON_RPC_REGISTER_API( API_NAME )                                                       \
{                                                                                               \
   taiyi::plugins::json_rpc::detail::register_api_method_visitor vtor( API_NAME );              \
   for_each_api( vtor );                                                                        \
}
...
namespace taiyi { namespace plugins { namespace json_rpc {
    ...
    public:
        ...
        static const std::string& name() { static std::string name = TAIYI_JSON_RPC_PLUGIN_NAME; return name; }
    ...
    namespace detail
    {
        ...
        {
        public:
            register_api_method_visitor( const std::string& api_name ) : _api_name( api_name ), _json_rpc_plugin( appbase::app().get_plugin< taiyi::plugins::json_rpc::json_rpc_plugin >() )
            {}
            ...
            void operator()(Plugin& plugin, const std::string& method_name, Method method, Args* args, Ret* ret )
            {
                _json_rpc_plugin.add_api_method(
                    _api_name,
                    method_name,
                    [&plugin,method]( const fc::variant& args ) -> fc::variant { return fc::variant( (plugin.*method)( args.as< Args >(), true ) ); },
        ...
        private:
            ...
            taiyi::plugins::json_rpc::json_rpc_plugin& _json_rpc_plugin;
        };
        
    } //detail

} } } // taiyi::plugins::json_rpc

FC_REFLECT( taiyi::plugins::json_rpc::api_method_signature, (args)(ret) )
