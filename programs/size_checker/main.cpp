...
int main( int argc, char** argv )
{
   try
   {
      ...
      std::vector<uint64_t> simings; simings.resize(50);
      for( uint32_t i = 0; i < 60*60*24*30; ++i )
      {
         simings[ rand() % 50 ]++;
      }

      std::sort( simings.begin(), simings.end() );
      idump((simings.back() - simings.front()) );
      idump((60*60*24*30/50));
      idump(("deviation: ")((60*60*24*30/50-simings.front())/(60*60*24*30/50.0)));
