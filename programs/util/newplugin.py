...
target_link_libraries( {plugin_provider}_{plugin_name} taiyi_app taiyi_chain taiyi_protocol )
...
{{
   public:
      {plugin_name}_api( const taiyi::app::api_context& ctx );
...
class {plugin_name}_plugin : public taiyi::app::plugin
{{
   public:
      {plugin_name}_plugin( taiyi::app::application* app );
...
{{
   public:
      {plugin_name}_api_impl( taiyi::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      taiyi::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( taiyi::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const taiyi::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( taiyi::app::application& app );
      ...
      taiyi::app::application& _app;
      boost::signals2::scoped_connection _applied_block_conn;
}};

{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( taiyi::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( taiyi::app::application* app )
   : plugin(app)
{{
...
TAIYI_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider (taiyi for plugins developed by zuowangdao)")
