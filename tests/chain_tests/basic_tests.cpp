...
BOOST_AUTO_TEST_CASE( adjust_balance_test )
{
    ...
    BOOST_TEST_MESSAGE( " --- Testing adding YANG_SYMBOL" );
    db->adjust_balance( "alice", asset( 50000, YANG_SYMBOL ) );
    BOOST_REQUIRE( db->get_balance( "alice", YANG_SYMBOL ) == asset( 50000, YANG_SYMBOL ) );
    
    BOOST_TEST_MESSAGE( " --- Testing deducting YANG_SYMBOL" );
    TAIYI_REQUIRE_THROW( db->adjust_balance( "alice", asset( -50001, YANG_SYMBOL ) ), fc::assert_exception );
    db->adjust_balance( "alice", asset( -30000, YANG_SYMBOL ) );
    db->adjust_balance( "alice", asset( -20000, YANG_SYMBOL ) );
    BOOST_REQUIRE( db->get_balance( "alice", YANG_SYMBOL ) == asset( 0, YANG_SYMBOL ) );
}
